...
class VideoIndexerService:
    def __init__(self):
        ...
        self.vi_name = os.getenv("AZURE_VI_NAME", "national-security-guardian-001") 
    ...
    # 🔥🔥 UPDATED FUNCTION (TIMESTAMP ENABLED)
    def extract_data(self, vi_json):
        """Parses the JSON into our State format WITH TIMESTAMPS."""
        ...
        for v in vi_json.get("videos", []):
            for insight in v.get("insights", {}).get("transcript", []):
                ...
                transcript_data.append({
                    ...
                    "start": instances[0].get("start"),   # 🔥 timestamp start
                    "end": instances[0].get("end")        # 🔥 timestamp end
                })

        # OCR extraction (same as before)
        ocr_lines = []
        for v in vi_json.get("videos", []):
        ...
        return {
            "transcript": transcript_data,   # 🔥 CHANGED: now structured
            "ocr_text": ocr_lines,
            "video_metadata": {
